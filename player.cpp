...
Player::Player(PlayerType t, Color c, const std::string& n, Vector2 pos, Vector2 sz,
...
static EdgeCollision CheckEdgeCollision(const Vector2& pos, const Vector2& size,
    ...
    float dotProduct = (center.x - closest.x) * normal.x +
                       (center.y - closest.y) * normal.y;
...
static int GetBestCollisionDirection(const Vector2& pos, const Vector2& size,
                                     const std::vector<Vector2>& poly,
                                     Vector2& pushPoint, Vector2& pushNormal,
                                     Vector2& edgeStart, Vector2& edgeEnd) {
...
void Player::Update(int leftkey, int rightkey, int upkey,
    ...
    if (velocity.x != 0) {
        while ((velocity.x > 0 && position.x < newX) ||
               (velocity.x < 0 && position.x > newX)) {
            position.x += stepX;
            bool hitWall = false;
            ...
            for (const auto& plat : platforms) {
                ...
                int dir = GetBestCollisionDirection(position, size, plat.points, 
                                                     pushPoint, pushNormal, edgeStart, edgeEnd);
                ...
                if (dir == 2 || dir == 3) {
                    ...
                    for (float h = 1.0f; h <= STEP_UP_MAX; h += 1.0f) {
                        ...
                        int dir2 = GetBestCollisionDirection(testPos, size, plat.points, 
                                                              tmpPoint, tmpNormal, tmpEdgeStart, tmpEdgeEnd);
    ...
    if (velocity.y != 0) {
        while ((velocity.y > 0 && position.y < newY) ||
               (velocity.y < 0 && position.y > newY)) {
            position.y += stepY;
        ...
        if (edgeLen > 0.0001f) {
            ...
            if (isHorizontal > 0.2f) {
                if (moveDir.x != 0) {
                    ...
                    if ((edgeVector.y < 0 && moveDir.x > 0) || 
                        (edgeVector.y > 0 && moveDir.x < 0)) {
                        velocity.x = moveDir.x * speed;
...
void Player::Draw() {
    if (isDead) {
        DrawRectangle(static_cast<int>(position.x), static_cast<int>(position.y),
                      static_cast<int>(size.x), static_cast<int>(size.y), GRAY);
    } else {
        DrawRectangle(static_cast<int>(position.x), static_cast<int>(position.y),
                      static_cast<int>(size.x), static_cast<int>(size.y), color);
    }
}